...
namespace ICTaximen.userControls
{
    ...
    {
       // ucTaximan taximan = new ucTaximan();
        public int IDMalade = -1;
        object[] data;
        clsImabar codage = new clsImabar();
        public ucTaximanform(object[] data = null)
        {
            InitializeComponent();
        ...
        private void Save(int index)
        {
            if (index == 0)
            {
                if (this.CheckFormFields())
                {
                    ...
                    {
                        ALLProjetctdll.Classes.clsGlossiaireMYSQL.GetInstance().saveData("saveTaximan", Constantes.TaximenDBChamps, values);
                        this.RefreshForm();
                       // taximan.ChargerGrid();
                    }
